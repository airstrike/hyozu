    ...
    Renderer: text::Renderer + geometry::Renderer,
{
        ...
        for (i, entry) in self.data.entries.iter().enumerate() {
            ...
            let box_layout = if self.data.show_box {
                entry.stats.as_ref().map(|stats| BoxLayout {
                    ...
                    median_y: plane
                        .to_pixel(Datum::new(i as f64, stats.median))
                        .y,
        ...
        Theme: crate::design::Design + ?Sized,
    {
        ...
        for (entry_idx, (entry_data, entry_layout)) in self
            .data
            .entries
            .iter()
            ...
            .enumerate()
        {
            ...
            stroke_frame.stroke(
                &shape,
                Stroke::default().with_width(1.0).with_color(base_color),
            );
            ...
            if let Some(box_layout) = &entry_layout.box_layout {
                ...
                let box_path = Path::new(|b| {
                    b.rectangle(
                        Point::new(cx - box_half, top),
                        Size::new(box_half * 2.0, height),
                    );
                });
