...
//!
//! - `width(Fill)` + `wrapping(None)` for single-line fit that scales to
//!   the available width
//! - `width(Fill).height(Fill)` + `wrapping(Word)` for multi-line fit that
//!   scales to both axes
//!
//! # Caveat
//!
...
use crate::core::{
    Color, Element, Em, Length, Pixels, Rectangle, Size, Widget, alignment, font, mouse, renderer, text,
};

pub use core_text::{Alignment, Catalog, Ellipsis, LineHeight, Shaping, Style, StyleFn, Wrapping};

/// Text that scales its font size to fit its laid-out bounds.
///
/// See the [module docs](self) for details.
#[must_use]
pub struct FitText<'a, Theme = crate::core::Theme, Renderer = crate::widget::Renderer>
where
    Theme: Catalog,
    ...
    Renderer: text::Renderer,
{
    ...
    /// unbounded within `[1.0, 1024.0]` pixels.
    ...
    /// Sets the font [`Weight`] of the [`FitText`] without changing the font family.
    ///
...
/// Everything a fit result is a function of. If all of these match the last
/// probe, we can reuse the previously chosen size.
#[derive(Clone)]
...
impl<Message, Theme, Renderer> Widget<Message, Theme, Renderer> for FitText<'_, Theme, Renderer>
where
    Theme: Catalog,
    Renderer: text::Renderer,
{
    ...
    fn layout(&mut self, tree: &mut Tree, renderer: &Renderer, limits: &layout::Limits) -> layout::Node {
        ...
        layout::sized(limits, self.format.width, self.format.height, |limits| {
            ...
            let fit_bounds = Size::new(
                if compression.width { f32::INFINITY } else { bounds.width },
            ...
            let chosen = match &state.cache {
                ...
                _ => {
                    ...
                    state.cache = Some(FitCache { chosen, ..probe_key });
                    chosen
                }
            };
...
fn fit<P: Paragraph>(
    ...
    let fits = |min_bounds: Size| -> bool {
        min_bounds.width <= fit_bounds.width + tol && min_bounds.height <= fit_bounds.height + tol
    };
...
impl<'a, Message, Theme, Renderer> From<FitText<'a, Theme, Renderer>> for Element<'a, Message, Theme, Renderer>
where
    Theme: Catalog + 'a,
    Renderer: text::Renderer + 'a,
{
    fn from(text: FitText<'a, Theme, Renderer>) -> Element<'a, Message, Theme, Renderer> {
        Element::new(text)
    }
}
